...
echo "<table id=\"output\" cellpadding=0 cellspacing=0 border=0 class=\"sortable-onload-1 rowstyle-alt colstyle-alt\">\n
  ...
  <thead>\n
    <tr>\n
      <th class=\"sortable-text\">Measure</th>\n
      <th class=\"sortable-numeric\">PLU</th>\n
...
while ($row = mysql_fetch_array ($result, MYSQL_ASSOC)) {
	// $bg = ($bg=='#eeeeee' ? '#ffffff' : '#eeeeee'); // Switch the background color.
		...
		if($row["scale"] == 1) { 
			echo '<td style="background-color : #ccc">SCALE</td>';
		}
		else { 
			echo '<td>EACH</td>';
		}
		...
		echo '
		<td align=right><a href="../index.php?q=' . $row["UPC"] . '">' . trim($row["UPC"], '0') . '</a></td>
...
echo '<form method = "post" action="plu_cashier.php" target="_blank">
